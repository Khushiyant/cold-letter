...
target_type = st.radio("What would you like to do?", ("Company", "Professor"))
...
requested_position = st.text_input(
    "Requested Position", placeholder="Software Engineer")
...
if st.button("Submit"):
    content = mailer.generate(target_name, name, extractor.extract(
        resume), target_type, requested_position)
    st.write(content)
